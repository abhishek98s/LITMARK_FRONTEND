...
import { Bookmark, bookmarkApiBody, bookmarkResponse, bookmarkArrayResponse, SearchResponse } from '../Model/bookmark.model';
...
export class BookmarkService {
  ...
  updateBookmarkname(id: number, title: string) {
    this.http.patch<bookmarkResponse>(`https://litmark-backend-2.vercel.app/api/bookmark/${id}`, { title }).subscribe({
      next: () => {
  ...
  deleteBookmark(id: number) {
    this.http.delete<bookmarkResponse>(`https://litmark-backend-2.vercel.app/api/bookmark/${id}`).subscribe({
      next: () => {
  ...
  moveBookmark(bookmarkId: number, nestedFolderId: number) {
    this.http.patch(`https://litmark-backend-2.vercel.app/api/bookmark/${bookmarkId}`, { folder_id: nestedFolderId }).subscribe({
      next: () => {
  ...
  getBookmarkThumbnail(image_id: number) {
    return this.http.get<bookmarkResponse>(`https://litmark-backend-2.vercel.app/api/image/${image_id}`);
  }

  searchBookmarkByTitle(title: string, folder_id: number) {
    return this.http.get<SearchResponse>(`https://litmark-backend-2.vercel.app/api/bookmark/search?title=${title}&folder_id=${folder_id}`)
  }

  onBookmarkClick(bookmarkId: number) {
    return this.http.patch(`https://litmark-backend-2.vercel.app/api/bookmark/recent/${bookmarkId}`, {})
  }
}
